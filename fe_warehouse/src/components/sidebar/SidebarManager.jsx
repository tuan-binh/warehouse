...
function SidebarManager({ toggle, handleToggleNavbar, expanded, handleChange }) {
  const link = [
    {
      id: 1,
      icon: <DescriptionOutlinedIcon className="nav_icon" />,
      subMenu: [
        { id: 1, path: "/manager/bills/export", text: "Danh Sách Phiếu Xuất" },
        { id: 2, path: "/manager/bills/import", text: "Danh Sách Phiếu Nhập" },
      ],
      text: "Đơn Hàng",
    },
    {
      ...
      subMenu: [
        { id: 1, path: "/manager/products", text: "Sản Phẩm Trong Kho" },
      ],
      text: "Sản Phẩm",
    },
    {
      ...
      subMenu: [
        { id: 1, path: "/manager/reports/inventory", text: "Báo Cáo Tồn Kho" },
        { id: 2, path: "/manager/reports/bill", text: "Báo Cáo Đơn Hàng" },
        { id: 3, path: "/manager/reports/revenue", text: "Báo Cáo Doanh Số" },
      ],
      text: "Báo Cáo",
    },
  ];
        ...
        <div>
          <div className="navbar__logo">
            {toggle ? (
              ...
              <Link to={"/manager"}>
                <HomeOutlinedIcon className="nav_icon" /> <b>{user && "KHO " + user.storageId}</b>
              </Link>
            )}
          </div>
          {toggle ? (
          ...
          <div className="nav_toggle">
            <ul className="nav__list">
              {
                active === "Tổng Quan" ?
                  <Link to={"/manager"} onClick={() => handleActiveSideBar("Tổng Quan")} className="nav__link active">
                    <DashboardOutlinedIcon className="nav_icon" />{" "}
                    {toggle ? "" : "Tổng Quan"}
                  </Link>
                  :
                  <Link to={"/manager"} onClick={() => handleActiveSideBar("Tổng Quan")} className="nav__link">
                    <DashboardOutlinedIcon className="nav_icon" />{" "}
                    {toggle ? "" : "Tổng Quan"}
                  </Link>
              }
