import { Button, Pagination, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Tooltip } from '@mui/material'
          ...
          <Table sx={{ width: '100%' }} aria-label="simple table">
            <TableHead>
              <TableRow>
                ...
                <TableCell align="center">NHÀY TẠO</TableCell>
            ...
            <TableBody>
              ...
              {inventory && inventory.map((item, index) => {
                return (
                  ...
                  >
                    <TableCell onClick={() => navigate(`/manager/reports/inventory/detail/${item.id}/${item.created}`)} align='center'>{index + 1}</TableCell>
                    <TableCell onClick={() => navigate(`/manager/reports/inventory/detail/${item.id}/${item.created}`)} align='center'>{item?.created} </TableCell>
                    <TableCell onClick={() => navigate(`/manager/reports/inventory/detail/${item.id}/${item.created}`)} align='center'>{item?.note}</TableCell>
                    <TableCell align='center'>
                      <Button onClick={() => handleExportExcelInventory(item.id)} variant="outlined" className='flex gap-2'> <FileUploadIcon />
