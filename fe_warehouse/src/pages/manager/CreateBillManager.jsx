...
function CreateBillManager() {
  ...
  const handleAddBill = () => {
    if (bill) {
      ...
      if (!bill.shipmentId) {
        setErrorShipmemt("Vui Lòng Chọn chọn đơn vị vận chuyển")
        return;
      }
            ...
            {userRecipient ?
              ...
              (<div className="search relative">
                <TextField
                  ...
                  value={searchRecipients} fullWidth
                  label={errorEndStorage ? errorEndStorage : "Nơi nhận hàng"}
                  variant="outlined"
                />
                {searchRecipients &&
                  <div className='absolute w-full mt-2 bg-slate-100 p-1 shadow-md'>
                    {
                      storage && storage.map((item) => {
                        return (item.id !== myStorage.id ? <p key={item.id} onClick={() => { setErrorEndStorage(''); setUserRecipient(item); dispatch(setEndStorageId(item.id)) }} className='p-2 m-2 bg-white shadow-md hover:cursor-pointer'>{item.storageName}</p> : '')
                      })
                    }
                  </div>
                }
          ...
          <div className='flex-1 flex gap-2'>
            ...
            <div className='relative flex-1'>
              <TextField
                ...
                size='small'
                label={errorProducts ? errorProducts : "Tìm kiếm theo tên hoặc mã sản phẩm"}
                variant="outlined"
              />
              {searchProduct &&
                (
...
export default CreateBillManager;