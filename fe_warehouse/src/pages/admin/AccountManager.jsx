...
function AccountManager() {
  ...
  const handleExportExcel = async () => {
    exportExcel("/api/v1/users/export/manager", "user_manager");
  }
    ...
    <div>
      <div className='flex justify-between'>
        <div className="actions">
          <Button onClick={handleExportExcel} variant="contained" className='flex gap-2'> <FileUploadIcon /> <span>Xuất File Excel</span> </Button>
        </div>
      ...
      <div className="content  w-full mt-5 ">
        <div className="header bg-white p-6 shadow-md">
          <TextField id="outlined-basic" size='small' onChange={debouncing(hadnleChangeSearch, TIME_OUT)} fullWidth label="Tìm kiếm theo email nhận viên kho" name='search' variant="outlined" />
        </div>
            ...
            <Table sx={{ width: '100%' }} aria-label="simple table">
              ...
              <TableBody>
                {users && users.map((item, index) => {
                  return (item?.roles === 'ROLE_MANAGER' ? <TableRow
                    key={item.id}
                    ...
                    <TableCell align='center'>
                      {item.status ?
                        <div className="flex gap-2 justify-center">
                          <Button onClick={() => handleEdit(item.id)} color="warning" variant="contained"><Tooltip title='edit'><EditIcon /></Tooltip></Button>
                          <Button onClick={() => handleChangeStatusUser(item.id)} color="error" variant="contained"><Tooltip title='lock'><LockOutlinedIcon /></Tooltip></Button>
                        </div>
                        :
